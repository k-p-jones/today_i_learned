...
RSpec.describe 'Home page', type: :feature do
  ...
  let!(:post_2)  { FactoryBot.create(:post, user_id: user_2.id, created_at: Date.today - 9) }
  ...
  context 'with tags' do
    before(:each) do
      post_2.tag_list = 'javascript'
      post_4.tag_list = 'javascript'
    end

    it 'filters the results by tag name' do
      visit root_path
      click_link '#javascript'

      expect(page).to have_content('Displaying posts tagged with "javascript"')
      expect(page).to have_link(post_4.title, href: post_path(post_4.id))
      expect(page).to have_content(post_4.formatted_date)
      expect(page).to have_link(post_2.title, href: post_path(post_2.id))
      expect(page).to have_content(post_2.formatted_date)
      expect(page).to_not have_content(post_11.title)
      expect(page).to_not have_content(post_10.title)
      expect(page).to_not have_content(post_9.title)
      expect(page).to_not have_content(post_8.title)
      expect(page).to_not have_content(post_7.title)
      expect(page).to_not have_content(post_6.title)
      expect(page).to_not have_content(post_5.title)
      expect(page).to_not have_content(post_3.title)
